(function (global) {
	...
	function sortUrls(urlList){
		const sorted = urlList.slice().sort((a, b) => {
			const aFirst = getExt(a)[0] || '';
			const bFirst = getExt(b)[0] || '';
	...
	function pageLoading({onProgress, onComplete, autoHide=true, showOnStart=true, detectVideo=true, videoEvent='loadeddata'} = {}) {
		...
		if(autoHide){
			setTimeout(()=>{
				if (loadedCount < total) {
					console.warn('資源載入超時，自動隱藏 loading 畫面');
					hideLoading();
					onComplete?.(sortUrls(urls));
				}
			}, 15000);
